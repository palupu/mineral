...
class MPCDMS(Agent):
    """Model Predictive Control with Direct Multiple Shooting Agent."""
    
    def __init__(self, full_cfg, **kwargs):
        # Required attributes before calling super().__init__() - these are checked by the base Agent class
        self.network_config = full_cfg.agent.network  # Network configuration (not used in MPC but required)
        self.mpcdms_config = full_cfg.agent.mpcdms    # MPC-specific configuration
        self.num_actors = self.mpcdms_config.num_actors  # Number of parallel environments
        self.max_agent_steps = int(self.mpcdms_config.max_agent_steps)  # Maximum training steps
        
        # MPC parameters - prediction horizon and time step for discretization
        self.N = self.mpcdms_config.get('N', 40)  # prediction horizon (number of time steps)
        self.DT = self.mpcdms_config.get('DT', 0.05)  # time step for RK4 integration
        
        # System dimensions - pendulum has 1 control input and 2 state variables
        self.CONTROL_DIM = 1  # torque control input
        self.STATE_DIM = 2    # [theta, theta_dot] state variables
        self.DIM = self.CONTROL_DIM + self.STATE_DIM  # combined state-control dimension
        
        # Cost function parameters - weights for different terms in the objective
        self.COST_THETA = self.mpcdms_config.get('COST_THETA', 1.0)      # weight for angle cost
        self.COST_THETA_DOT = self.mpcdms_config.get('COST_THETA_DOT', 0.1)  # weight for angular velocity cost
        self.COST_CONTROL = self.mpcdms_config.get('COST_CONTROL', 0.001)    # weight for control effort cost
        self.COST_TERMINAL = self.mpcdms_config.get('COST_TERMINAL', 3.0)    # weight for terminal state cost
        
        # Physical parameters of the pendulum system
        self.GRAVITY = self.mpcdms_config.get('GRAVITY', 10.0)  # gravitational acceleration
        self.LENGTH = self.mpcdms_config.get('LENGTH', 1.0)     # pendulum length
        self.MASS = self.mpcdms_config.get('MASS', 1.0)         # pendulum mass
        
        # Optimization parameters
        self.MAX_ITER = self.mpcdms_config.get('MAX_ITER', 100)  # maximum iterations for scipy.minimize
    ...
    def train(self):
        ...
        while self.agent_steps < self.max_agent_steps:
            ...
            self.set_eval()  # set to evaluation mode (no-op for MPC)
            trajectory, steps = self.explore_env(self.env, self.mpcdms_config.get('horizon_len', 100))
    ...
    def eval(self):
        """Evaluation loop - runs the MPC controller without exploration."""
        self.set_eval()  # set to evaluation mode
        ...
        total_eval_episodes = self.num_actors * 2  # evaluate multiple episodes
        eval_metrics = self._create_metrics(total_eval_episodes, self.metrics_kwargs)
        
        # Run evaluation episodes
        with self._as_metrics(eval_metrics), torch.no_grad():  # use separate metrics, no gradients
            while self.metrics.num_episodes < total_eval_episodes:
                if not self.env_autoresets:
                    raise NotImplementedError

                # Generate actions deterministically (no exploration)
            ...
            self.metrics.flush_video(self.epoch)  # save evaluation video
    ...
    def set_eval(self):
        """Set to evaluation mode - no-op for MPC since it has no trainable networks."""
        # MPC doesn't have trainable networks, so this is a no-op
        pass
    
    def save(self, f):
        """Save agent state - saves MPC optimization variables."""
        # Save MPC-specific state for checkpointing
        ckpt = {
            'x_u': self.x_u,           # current optimization variables
            'bounds': self.bounds,     # optimization bounds
            'epoch': self.epoch,       # current epoch
            'agent_steps': self.agent_steps,  # total steps taken
        }
        torch.save(ckpt, f)
    
    def load(self, f):
        """Load agent state - restores MPC optimization variables."""
        # Load checkpoint from file
        ckpt = torch.load(f, map_location=self.device)
        self.x_u = ckpt.get('x_u')           # restore optimization variables
        self.bounds = ckpt.get('bounds')     # restore bounds
        self.epoch = ckpt.get('epoch', -1)   # restore epoch
        self.agent_steps = ckpt.get('agent_steps', 0)  # restore step count