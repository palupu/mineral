...
class DSSMPCAgent(Agent):
    ...
    def get_shac_initial_sequence(self, obs: dict, horizon: int) -> torch.Tensor:
        ...
        self.env.no_grad = True  # Disable gradients during rollout (just for initial guess)

        try:
            with torch.no_grad():
                # Normalize initial observation if needed (don't update normalizer during inference)
                if self.shac_agent.obs_rms is not None:
                    current_obs = {}
                    for k, v in obs.items():
                        if k in self.shac_agent.obs_rms:
                            # Check for NaN/inf values
                            if torch.isnan(v).any() or torch.isinf(v).any():
                                print(f"Warning: NaN/Inf in observation key '{k}', using zeros")
                                v = torch.zeros_like(v)
                            current_obs[k] = self.shac_agent.obs_rms[k].normalize(v)
                        else:
                            current_obs[k] = v
                else:
                    current_obs = obs
                ...
                for step in range(horizon):
                    ...
                    try:
                        ...
                        if torch.isnan(action).any() or torch.isinf(action).any():
                            print(f"Warning: NaN/Inf in action at step {step}, using zeros")
                    ...
                    # Convert and normalize next observation (don't update normalizer)
                    next_obs = self._convert_obs(next_obs)
                    if self.shac_agent.obs_rms is not None:
                        # Only normalize, don't update (use statistics from checkpoint)
                        current_obs_dict = {}
                        for k, v in next_obs.items():
                            if k in self.shac_agent.obs_rms:
                                # Check for NaN/inf values
                                if torch.isnan(v).any() or torch.isinf(v).any():
                                    print(f"Warning: NaN/Inf in observation key '{k}' at step {step}, using zeros")
                                    v = torch.zeros_like(v)
                                current_obs_dict[k] = self.shac_agent.obs_rms[k].normalize(v)
                            else:
                                current_obs_dict[k] = v
                    else:
    ...
    def single_shooting_plan(self, u_init: torch.Tensor = None, obs: dict = None) -> torch.Tensor:
        ...
        if u_init is not None:
            ...
            shac_u_init = self.get_shac_initial_sequence(obs, self.N)
