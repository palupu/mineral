...
class DSSMPCAgent(Agent):
    ...
    def single_shooting_loss(self, u_seq: torch.Tensor, init_state: Any, debug: bool = False) -> torch.Tensor:
        """Single shooting loss: negative cumulative reward from rollout.

        Rolls out the simulation from init_state using control sequence u_seq,
        accumulates rewards, and returns negative total (for minimization).

        Args:
            u_seq: Control sequence, shape (N, control_dim)
            init_state: Initial warp state to restore from
            debug: If True, print gradient tracking debug info

        Returns:
            loss: Negative cumulative reward (scalar tensor)
        """
        # Restore environment state and initialize gradient tracking (like BPTT does)
        self.env.state_0 = init_state
        self.env.initialize_trajectory()  # Cuts off old gradients, starts fresh
        ...
        for i in range(self.N):
            ...
            if debug and i == 0:
                ...
                print(f"[DEBUG]   grad(reward[0], actions) = {test_grad}")
                print("[DEBUG]   Is gradient actually zero, or just very small?")
                print(f"[DEBUG]   reward value: {reward[0].item()}")
                print(f"[DEBUG]   actions value: {actions}")
    ...
    def single_shooting_plan(self, u_init: torch.Tensor = None) -> torch.Tensor:
        ...
        # Save initial state for rollouts (clone to preserve it)
        ...
        for iteration in range(self.max_iter):
            ...
            # This restores init_state and rolls out with gradient tracking
            debug = (iteration == 0)  # Only print debug info on first iteration
            loss = self.single_shooting_loss(u_param, init_state, debug=debug)
        ...
        u_opt = u_param.data.clone()

        # Restore initial state for actual execution
        self.env.state_0 = init_state
    ...
    def run_single_shooting_mpc(self) -> None:
        ...
        # Let the dough fall briefly (reduced from 25 to 5 steps)
        # Too much stabilization means actions have no effect
        print("\nLetting dough fall briefly (5 steps)...")
        zero_action = torch.zeros((self.num_actors, self.control_dim), device=self.device)
        for _ in range(5):
            obs, reward, done, _ = self.env.step(zero_action)
            self.obs = self._convert_obs(obs)
            self.dones = done
        print("Brief stabilization complete.\n")
