...
class DSSMPCAgent(Agent):
    ...
    def run_single_shooting_mpc(self) -> None:
        ...
        self.obs_sapo = self._convert_obs(obs)
        ...
        for timestep in range(self.timesteps):
            ...
            # Just use SAPO policy
            # if self.shac_agent.obs_rms is not None:
            #     current_obs_sapo = {k: self.shac_agent.obs_rms[k].normalize(v) for k, v in current_obs.items()}
            # u_opt = self.shac_agent.get_actions(current_obs_sapo, sample=self.sapo_sample)
            # Compare total reward with SHAC policy. Set seed equal.
