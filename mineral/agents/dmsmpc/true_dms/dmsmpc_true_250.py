...
class TrueDMSMPCAgent(Agent):
    ...
    def _obs_to_state(self, obs):
        """Convert observation to full state vector (all MPM particle positions).

        For scientific research, we use the complete state: all particle positions.
        State vector: [x1, y1, z1, x2, y2, z2, ..., xN, yN, zN] for N particles
        """
        if isinstance(obs, dict):
            # Extract particle positions - this is the full state
            particle_q = obs.get('particle_q', None)
            ...
            if particle_q is not None:
                ...
                # Handle batch dimension: shape is (num_envs, num_particles, 3)
                # We only use the first environment
                if particle_q.ndim == 3:
                    particle_q = particle_q[0]  # Shape: (num_particles, 3)
                elif particle_q.ndim != 2:
                    raise ValueError(f"Expected particle_q with 2 or 3 dims, got {particle_q.ndim}")

                # Flatten to 1D: (num_particles, 3) -> (num_particles * 3,)
                state = particle_q.flatten()

                # Initialize dimensions if this is the first call
                if self.state_dim is None:
                    self.num_particles = particle_q.shape[0]
                    self.state_dim = self.num_particles * 3
                    self.dim = self.state_dim + self.control_dim
                    print(f"\n{'=' * 70}")
                    print("State Dimensions Initialized:")
        ...
        raise ValueError("Unable to extract particle_q from observation")
    ...
    def _set_state_from_vector(self, state_vec, template_state):
        ...
        if hasattr(new_state, 'mpm_x') and new_state.mpm_x is not None:
            # Convert to torch, assign to warp array
