...
class TrueDMSMPCAgent(Agent):
    ...
    - 'joint_com': Set joints + translate MPM particles to match COM - EXPERIMENTAL
    """
    ...
    def __init__(self, full_cfg, **kwargs):
        ...
        self.state_setting_strategy = self.dms_mpc_params.get('state_setting_strategy', 'joint_only')
        # Options: 'joint_only' (recommended), 'joint_com' (experimental)
    ...
    def _obs_to_state(self, obs):
        ...
        if isinstance(obs, dict):
            # Extract joint_q and com_q from observations
    ...
    def _compute_current_com(self, state):
        """Compute center of mass from MPM particle positions.
        
        Returns:
            com: numpy array of shape (3,) with COM position
        """
        if hasattr(state, 'mpm_x') and state.mpm_x is not None:
            # Convert warp array to numpy
            particles = wp.to_torch(state.mpm_x).cpu().numpy()
            # Compute mean position (assuming equal mass particles)
            com = np.mean(particles, axis=0)
            return com
        else:
            # No MPM particles, return zeros
            return np.zeros(3)

    def _set_state_from_vector(self, state_vec, template_state):
        """Set full environment state from reduced state vector.
        ...
        Args:
            state_vec: Reduced state vector, shape (state_dim,)
                      For RollingPin: [joint_q (3), com_q (3)] = 6D
            template_state: Full simulation state to use as template
                           (contains all MPM particles, grid, etc.)
        
        Returns:
            new_state: Full simulation state with updated reduced state
        
        Strategy Options:
        - 'joint_only': Only update joint_q, ignore com_q
          Pros: Simple, reliable, fast
          Cons: Can't directly control COM position
          
        - 'joint_com': Update joint_q + translate MPM particles for COM
          Pros: Controls both joints and COM
          Cons: Expensive (updates thousands of particles), may be unstable
        """
        # Clone full state (including all MPM particles!)
        new_state = self.clone_state(template_state)
        
        # Extract components from state vector
        if self.state_dim >= 3:
            joint_q = state_vec[:3]  # First 3 elements: joint positions
        else:
            joint_q = state_vec
            
        if self.state_dim >= 6:
            com_q = state_vec[3:6]   # Next 3 elements: COM position
        else:
            com_q = None

        # STRATEGY 1: Joint Only (Recommended)
        if self.state_setting_strategy == 'joint_only':
            # Only set joint positions
            if hasattr(new_state, 'joint_q') and new_state.joint_q is not None:
                joint_q_wp = wp.from_numpy(joint_q.astype(np.float32), device=self.device)
                new_state.joint_q.assign(joint_q_wp)
            # COM will be implicitly determined by physics
            
        # STRATEGY 2: Joint + COM via Particle Translation (Experimental)
        elif self.state_setting_strategy == 'joint_com' and com_q is not None:
            # Set joint positions
            if hasattr(new_state, 'joint_q') and new_state.joint_q is not None:
                joint_q_wp = wp.from_numpy(joint_q.astype(np.float32), device=self.device)
                new_state.joint_q.assign(joint_q_wp)
            
            # Translate all MPM particles to match desired COM
            if hasattr(new_state, 'mpm_x') and new_state.mpm_x is not None:
                current_com = self._compute_current_com(new_state)
                delta_com = com_q - current_com
                
                # Translate all particles
                particles = wp.to_torch(new_state.mpm_x)
                particles += torch.from_numpy(delta_com).float().to(self.device)
                new_state.mpm_x.assign(wp.from_torch(particles))
        
        else:
            raise ValueError(f"Unknown state_setting_strategy: {self.state_setting_strategy}")
    ...
    def cost(self, x_u, current_x):
        ...
        Computes standard quadratic cost: sum_i (x[i]^T Q x[i] + u[i]^T R u[i]) + x[N]^T Q_N x[N]
        """
    ...
    def eq_constraint(self, x_u, current_x, template_state):
        ...
        for i in range(self.N):
            ...
            constraint_violation = x[i + 1] - next_state_sim
            constraints.extend(constraint_violation)
    ...
    def dms_plan(self, current_x, init_state):
        ...
        print(f"\n{'='*70}")
        print(f"Starting TRUE DMS Optimization")
        print(f"  Strategy: {self.state_setting_strategy}")
